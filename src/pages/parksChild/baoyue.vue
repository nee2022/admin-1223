    ...
    <div>
      <template>
        <el-table :data="menuList" stripe style="width: 100%">
          ...
          <el-table-column prop="amount" label="套餐金额"> </el-table-column>
          <el-table-column prop="pay" label="套餐价格"> </el-table-column>
...
export default {
  ...
  methods: {
    ...
    chargersUser() {
      ...
      this.$refs.addFormRef.validate(valid => {
        if (!valid) {
          ...
          this.$axios.post("/admin/api/plan", this.addForm).then(res => {
            if (res.status !== 200) {
    ...
    async removeUserByID(id) {
      ...
      this.$axios
        .delete("/admin/api/plan/" + id + "?token=" + toKen) //,{"token:toKen"}
        .then(res => {
          console.log(res);
          if (res.status == 200) {
    ...
    showEditDialog(id) {
      ...
      // console.log(toKen)
      this.$axios
        .get("/admin/api/plan/" + id + "?token=" + toKen) //根据id点击修改的id查询设备信息
        .then(res => {
    ...
    editChargerInfo() {
      ...
      this.$refs.editFormRef.validate(valid => {
        if (!valid) {
          ...
          if (confirm("确认修改吗?")) {
            this.$axios
              .put("admin/api/plan/" + this.editForm.id, {
                token: toKen,
