          ...
          <el-table-column prop="address" label="操作" width="200">
            <template slot-scope="scope">
              <div class="operation">
                <div>
                  <img
                    src="../../assets/images/top-up.png"
                    @click="(addDialogVisible = true), getID(scope.row.id)"
                    alt="充值"
                    title="充值"
                  />
                </div>
    ...
    Recharge() {
      ...
      this.$refs.addFormRef.validate(valid => {
        if (!valid) {
          ...
          this.$axios
            .post("admin/api/user/" + this.recharID + "/charge", this.addForm)
            .then(res => {
              if (res.status !== 200) {
